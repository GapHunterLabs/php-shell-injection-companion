package dev.gaphunter.phpshellinjectioncompanion.inspection
...
import dev.gaphunter.phpshellinjectioncompanion.detect.ShellInjectionScanner
import dev.gaphunter.phpshellinjectioncompanion.review.ReviewPrompt

/**
 * Flags `eval`/`exec`/`shell_exec`/`system`/`passthru`/`popen` calls
 * with an interpolated or concatenated command string -- see
 * [ShellInjectionScanner] for the full reasoning. Runs via
    ...
    override fun checkFile(file: PsiFile, manager: InspectionManager, isOnTheFly: Boolean): Array<ProblemDescriptor>? {
        ...
        for (hit in hits) {
            ...
            problems += manager.createProblemDescriptor(
                anchor,
                relativeRange,
                "${hit.callText}(...) runs an interpolated/concatenated string -- if any part comes from " +
                    "untrusted input, this is a command/code injection vulnerability (CWE-78/CWE-95). PHP's own " +
                    "manual warns not to pass unvalidated user data into these functions",
